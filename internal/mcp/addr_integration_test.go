...
// TestScanRequestNoParamsAddressPolicyAction verifies that response-shaped
// messages (no params) use the address-specific action, not the MCP input action.
// Regression test: the no-params path previously stamped Action: action (MCP input),
// bypassing a stricter address_protection.action.
func TestScanRequestNoParamsAddressPolicyAction(t *testing.T) {
	...
	f := false
	cfg.AddressProtection.Enabled = true
	cfg.AddressProtection.Action = config.ActionBlock // address protection wants block
	cfg.AddressProtection.UnknownAction = testUnknownActionAllow
	cfg.AddressProtection.AllowedAddresses = []string{
		"0x742d35cc6634c0532925a3b844bc9e7595f2bd3e",
	}
	...
	if len(verdict.AddressFindings) == 0 {
		t.Fatal("no-params path: should have address findings")
	}
