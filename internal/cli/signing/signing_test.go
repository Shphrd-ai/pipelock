...
func TestSignCmd_SaveSignatureError(t *testing.T) {
	...
	base := t.TempDir()
	ks := domsigning.NewKeystore(base)
...
// TestPipelockHomeEnvKeygen tests that PIPELOCK_HOME env var is used as
// the keystore directory when no --keystore flag is given.
// This replaces the original TestHomeFlagKeygen which tested the --home
// persistent flag on the root command (not available in this subpackage).
func TestPipelockHomeEnvKeygen(t *testing.T) {
	homeDir := t.TempDir()
	t.Setenv("PIPELOCK_HOME", homeDir)

	cmd := testRoot()
	cmd.SetArgs([]string{"keygen", "homer"})
	buf := &strings.Builder{}
	cmd.SetOut(buf)
...
// TestPipelockHomeEnvSignVerify tests sign/verify using PIPELOCK_HOME env var.
func TestPipelockHomeEnvSignVerify(t *testing.T) {
	homeDir := t.TempDir()
	t.Setenv("PIPELOCK_HOME", homeDir)

	// Generate key using PIPELOCK_HOME.
	ks := domsigning.NewKeystore(homeDir)
	...
	// Sign with PIPELOCK_HOME (no --keystore).
	signC := testRoot()
	signC.SetArgs([]string{"sign", testFile, "--agent", "homer"})
	signC.SetOut(&strings.Builder{})
	if err := signC.Execute(); err != nil {
		t.Fatalf("sign with PIPELOCK_HOME error: %v", err)
	}

	// Verify with PIPELOCK_HOME (no --keystore).
	verifyC := testRoot()
	verifyC.SetArgs([]string{"verify", testFile, "--agent", "homer"})
	buf := &strings.Builder{}
	verifyC.SetOut(buf)
	if err := verifyC.Execute(); err != nil {
		t.Fatalf("verify with PIPELOCK_HOME error: %v", err)
	}
	if !strings.Contains(buf.String(), "OK") {
		t.Errorf("expected OK, got: %s", buf.String())
	}
...
// TestPipelockHomeEnvTrust tests trust using PIPELOCK_HOME env var.
func TestPipelockHomeEnvTrust(t *testing.T) {
	homeDir := t.TempDir()
	t.Setenv("PIPELOCK_HOME", homeDir)
	...
	cmd := testRoot()
	cmd.SetArgs([]string{"trust", "remote-agent", pubKeyPath})
	buf := &strings.Builder{}
	cmd.SetOut(buf)

	if err := cmd.Execute(); err != nil {
		t.Fatalf("trust with PIPELOCK_HOME error: %v", err)
	}
	if !strings.Contains(buf.String(), "Trusted") {
		t.Errorf("expected 'Trusted' in output, got: %s", buf.String())
	}

	// Verify the trusted key is stored under PIPELOCK_HOME.
	_, err := ks.LoadTrustedKey("remote-agent")
	if err != nil {
		t.Fatalf("trusted key not loadable under PIPELOCK_HOME dir: %v", err)
	}
}

// TestKeystoreOverridesEnv verifies --keystore takes precedence over PIPELOCK_HOME.
func TestKeystoreOverridesEnv(t *testing.T) {
	homeDir := t.TempDir()
	keystoreDir := t.TempDir()
	t.Setenv("PIPELOCK_HOME", homeDir)

	cmd := testRoot()
	cmd.SetArgs([]string{"keygen", "precedence", "--keystore", keystoreDir})
	cmd.SetOut(&strings.Builder{})

	if err := cmd.Execute(); err != nil {
		t.Fatalf("keygen with --keystore and PIPELOCK_HOME error: %v", err)
	}

	// Keys should be in --keystore, NOT in PIPELOCK_HOME.
	...
	if _, err := os.Stat(homePriv); !os.IsNotExist(err) {
		t.Errorf("private key should NOT be in PIPELOCK_HOME dir when --keystore is set")
	}
}

// TestKeygenCmd_HomeFlag verifies that --home threads through to the keystore
// directory. Regression test: the CLI split must not break --home resolution.
func TestKeygenCmd_HomeFlag(t *testing.T) {
	homeDir := t.TempDir()
	...
	cmd.SetOut(&buf)
	cmd.SetArgs([]string{"test-agent", "--force"})
	...
	output := buf.String()
	if !strings.Contains(output, homeDir) {
		t.Errorf("expected output to reference --home dir %q, got:\n%s", homeDir, output)
	}

	// Verify key was actually written under the home directory.
	pubKey := filepath.Join(homeDir, "agents", "test-agent", "id_ed25519.pub")
	if _, err := os.Stat(pubKey); err != nil {
		t.Errorf("public key not found at %q: %v", pubKey, err)
	}
}
...
func TestResolveKeystoreDir_EnvFallback(t *testing.T) {
	...
	result, err := cliutil.ResolveKeystoreDir("")
	if err != nil {
...
func TestResolveKeystoreDir_ExplicitOverridesEnv(t *testing.T) {
	homeDir := t.TempDir()
	explicit := t.TempDir()
	t.Setenv("PIPELOCK_HOME", homeDir)

	result, err := cliutil.ResolveKeystoreDir(explicit)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if result != explicit {
		t.Errorf("expected explicit %q to override PIPELOCK_HOME %q, got %q", explicit, homeDir, result)
	}
}
