...
// preflightRoot creates a minimal root command with the preflight subcommand
// registered, matching how it would appear in the real CLI.
func preflightRoot() *cobra.Command {
	root := &cobra.Command{
		Use:           "pipelock",
		SilenceErrors: true,
		SilenceUsage:  true,
...
func TestPrintPreflightReport_NoFindings(t *testing.T) {
	...
	if !containsStr(buf.String(), "No findings") {
...
func TestPreflight_Malicious(t *testing.T) {
	...
	output := buf.String()
	if !containsStr(output, "[CRITICAL]") {
		t.Error("expected [CRITICAL] in text output")
	}
	if !containsStr(output, "[HIGH]") {
