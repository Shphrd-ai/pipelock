...
func (c *Config) validateConductor(warnings *[]Warning) error {
	...
	// A pinned trust-roster root fingerprint is mandatory whenever conductor is
	// enabled, INDEPENDENT of honor_remote_kill_switch. The follower verifies
	// signed policy bundles (and, when honored, remote-kill messages) against
	// the roster rooted at this fingerprint; without it the follower could apply
	// unverified signed control-plane input. honor_remote_kill_switch only
	// governs whether remote-kill STATE is applied, not whether trust material
	// is required.
	if strings.TrimSpace(cfg.TrustRosterRootFingerprint) == "" {
		return fmt.Errorf("conductor.trust_roster_root_fingerprint required when conductor.enabled is true")
	}
